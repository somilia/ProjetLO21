...
Regle *ajouteEnQueueRegle(Regle* regle, char *prop){
    ...
    regle->nbpropo++;
    regle->premier= ajouteEnQueueRec(regle->premier, prop);
    return regle;
}

Proposition *ajouteEnQueueRec(Proposition *regle, char *nvProp){ //Ajoute récursivement une proposition en queue
    ...
    regle->suivant = ajouteEnQueueRec(regle->suivant, nvProp);
    return regle;
